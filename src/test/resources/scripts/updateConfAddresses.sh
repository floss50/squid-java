...
DISPENSER_ADDRESS=$(jq -r .address "${HOME}/.ocean/keeper-contracts/artifacts/Dispenser.spree.json")
...
sed -i "s/contract.dispenser.address=.*/contract.dispenser.address=\"$DISPENSER_ADDRESS\"/g" src/test/resources/application.conf
