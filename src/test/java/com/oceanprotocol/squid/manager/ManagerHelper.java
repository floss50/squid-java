...
public abstract class ManagerHelper {
    ...
    /**
     * Usuful method for debugging purposes
     * @param serviceAgreementId
     * @param saContract
     * @param keeper
     * @throws IOException
     */
    public static void checkExecuteAgreementEvents(String serviceAgreementId, ServiceAgreement saContract, KeeperDto keeper) throws IOException {
        ...
        */
        ...
        /*
        Check all executeAgreements events
        EthFilter didFilter = new EthFilter(
                DefaultBlockParameterName.EARLIEST,
                DefaultBlockParameterName.LATEST,
                saContract.getContractAddress()
        );

        final Event event= saContract.EXECUTEAGREEMENT_EVENT;
        final String eventSignature= EventEncoder.encode(event);
        didFilter.addSingleTopic(eventSignature);
        EthLog ethLog= keeper.getWeb3().ethGetLogs(didFilter).send();
        List<EthLog.LogResult> logs = ethLog.getLogs();

        for (EthLog.LogResult logResult: logs){

            EventValues eventsValues = EventsHelper.staticExtractEventParameters(event, ((EthLog.LogObject) logResult));

            List<Type> nonIndexed= eventsValues.getNonIndexedValues();
            List<Type> indexed= eventsValues.getIndexedValues();

            String serviceAgreement = EncodingHelper.toHexString(((Bytes32)indexed.get(0)).getValue());
            String templateID = EncodingHelper.toHexString(((Bytes32)indexed.get(1)).getValue());
            String did = EncodingHelper.toHexString(((Bytes32)indexed.get(2)).getValue());

            Boolean status = (Boolean) nonIndexed.get(0).getValue();
            String templateOwner = nonIndexed.get(1).getValue().toString();
            String consumer = nonIndexed.get(2).getValue().toString();;
            Boolean state = (Boolean) nonIndexed.get(3).getValue();

            log.debug("Exec.Ag Event: " + serviceAgreement + ", " + templateID + "," +
                    did + ", " + status + ", " + templateOwner + ", " + consumer + ", " + state);
...
*/
        if (serviceAgreementId != null) {
            ...
            final Event events= saContract.EXECUTEAGREEMENT_EVENT;
            final String eventSignatures= EventEncoder.encode(events);
            String slaTopic= "0x" + serviceAgreementId;
            slaFilter.addSingleTopic(eventSignatures);
            slaFilter.addOptionalTopics(slaTopic);

            EthLog ethLogs= keeper.getWeb3().ethGetLogs(slaFilter).send();
            List<EthLog.LogResult> logss = ethLogs.getLogs();

            if (logss.size()>0) {
                log.debug("Received event for service agreement");
            }
